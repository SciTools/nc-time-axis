...
class Test_compute_resolution(unittest.TestCase):
    ...
    def check(self, max_n_ticks, num1, num2):
        locator = NetCDFTimeDateLocator(
            max_n_ticks=max_n_ticks, calendar=self.calendar, date_unit=self.date_unit
        )
        utime = cftime.utime(self.date_unit, self.calendar)
        return locator.compute_resolution(
            num1, num2, utime.num2date(num1), utime.num2date(num2)
        )
    ...
    def test_10_years(self):
        ...
        self.assertEqual(
            self.check(10 * 365 * 86400, 0, 10 * 365), ("SECONDLY", mdates.SEC_PER_DAY)
        )
...
class Test_tick_values(unittest.TestCase):
    ...
    def test_secondly(self):
        np.testing.assert_array_almost_equal(
            self.check(4, 0, 0.0004), [0.0, 0.000116, 0.000231, 0.000347, 0.000463]
        )
